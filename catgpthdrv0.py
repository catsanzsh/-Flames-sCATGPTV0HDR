...
class AutonomousAgent:
    ...
    def run(self):
        ...
        self.log_to_ui(f"SYSTEM PROMPT INITIALIZED:\n{self.get_system_prompt()}", "llm") # Log the full initial prompt for debugging
...
class CatGPTFusion(tk.Tk):
    ...
    def launch_autonomous_agent_window(self, goal: str, system_prompt: str, selected_tool_names: List[str]):
        ...
        def stop_agent(): 
            if not stop_event.is_set(): # Prevent multiple calls if already stopping
                stop_event.set()
                if stop_button.winfo_exists(): # Check if button still exists
        ...
        def process_queue():
            try:
                ...
                while not ui_queue.empty():
                    ...
                    if win.winfo_exists(): # Check if window is still there
                        log_display.config(state=tk.NORMAL)
                        log_display.insert(tk.END, f"[{datetime.now().strftime('%H:%M:%S')}] ", ("info",)) # Default tag for timestamp
                        log_display.insert(tk.END, f"{msg['content']}\n\n", (msg['tag'],))
                        log_display.config(state=tk.DISABLED)
                        log_display.see(tk.END)
                
                # Check agent status and decide next action for the UI loop
                if stop_event.is_set(): # Agent has been signaled to stop or has stopped
                    if ui_queue.empty(): # All messages from agent are processed
                        if win.winfo_exists() and not getattr(win, '_shutdown_timer_started', False):
                            win._shutdown_timer_started = True # Prevent multiple timers
                            if stop_button.winfo_exists():
                                stop_button.config(text="AGENT STOPPED - CLOSING SHORTLY...", state=tk.DISABLED)
                            
                            log_display.config(state=tk.NORMAL)
                            log_display.insert(tk.END, "\n\n[SYSTEM] Agent finished or was stopped. Window will close automatically in 3 seconds.", ("header",))
                            log_display.config(state=tk.DISABLED)
                            log_display.see(tk.END)
                            
                            win.after(3000, lambda: win.destroy() if win.winfo_exists() else None)
                        return # Stop further calls to process_queue if shutting down
                
                # If agent is not stopped, or stopped but still has messages, reschedule
                if win.winfo_exists():
                    win.after(200, process_queue)

            except queue.Empty: # Should not happen with the while loop, but good practice
                if win.winfo_exists() and not getattr(win, '_shutdown_timer_started', False): # Only reschedule if not already shutting down
                    win.after(200, process_queue)
            except tk.TclError: # Window might have been destroyed
                logger.info("Mission Control window closed or TclError during UI update.")
                if not stop_event.is_set():
                    stop_event.set() # Ensure agent thread stops if UI is gone
            except Exception as e: 
                logger.error(f"Unexpected error in Mission Control process_queue: {e}\n{traceback.format_exc()}")
                if win.winfo_exists() and not getattr(win, '_shutdown_timer_started', False):
                     win.after(200, process_queue) # Try to continue if possible
        
        win.protocol("WM_DELETE_WINDOW", stop_agent) # Ensure stop_event is set if user closes window
        win.after(200, process_queue) # Start the UI update loop
